...
int main (int argc, char *argv[])
{
  ...
  int mrfLines;
    ...
    int a,k,numTxt, tempIntrasize;  //peter
  ...
  while (currMrfEntry = mrf_nextEntry ()) {
    mrfLines++;
    a++; // try to see how many times of the comparison
    ...
    for (i = 0; i < arrayMax (currMrfRead1->blocks); i++) {
      ...
      while (a>25000)
      {
          ...
          stringPrintf(buffer,"%s_intra_%d.txt",argv[1],numTxt);
          warn("save temp file: %s",string(buffer));
          fp=fopen(string (buffer), "w");
          for (i=0;i<arrayMax(intras);i++)
          {
              currIntra=arrayp (intras,i,Intra);
              fprintf (fp,"%p\t%d\t%d\t%d\t%d\t%s\t%s\n",currIntra->transcript,currIntra->readStart1,currIntra->readStart2, currIntra->readEnd1, currIntra->readEnd2, currIntra->read1, currIntra->read2);
          }
          fclose (fp);
          numTxt++;
          a=0;
    ...
    warn("hi~number of intra:%d ",arrayMax(intras));
    mrf_deInit ();
    ...
    for (k=0 ; k<numTxt; k++)
    {
        ...
        /////////////////////peter: read the splited intra files////////////////////////////////////
        
        stringPrintf(buffer,"%s_intra_%d.txt",argv[1],k);
        warn("read temp file: %s",string(buffer));
        if((fp=fopen(string(buffer),"r")) == NULL) {
            printf("Cannot open file.\n");
            exit(1);
        }
        int n = 0;
        while(fscanf(fp,"%p\t%d\t%d\t%d\t%d\t%s\t%s\n", &v, &w, &x, &y, &z, r1, r2) == 7){
            n++;
            currIntra = arrayp (intras,arrayMax (intras),Intra);
            currIntra->transcript = v;
            currIntra->readStart1 = w;
            currIntra->readStart2 = x;
            currIntra->readEnd1 = y;
            currIntra->readEnd2 = z;
            currIntra->read1 = r1;
            currIntra->read2 = r2;
        }
        fclose(fp);
        warn("number of n: %d ",n);
        warn("%d number of new intra2: %d ",k, arrayMax(intras));
        warn("%d number of superIntras: %d ",k, arrayMax(superIntras));
