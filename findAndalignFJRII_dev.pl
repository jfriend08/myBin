...
if ( !$readlength || !$directory || !$samFile || !$sortedBamFile || !$gfrFile){
    print "not enought parameters";
    print "usage: findAndalignFJRII.pl <read length> <directory/of/FJR/and/sorted.bam> <FJR.sam> <Alignmed.sorted.bam> <.fgr>\n";
    print "example: findAndalignFJRII.pl 75 /aslab_scratch001/asboner_dat/PeterTest/test_Samples/testIII/ AS182_FJR.sam AS182_sorted.bam AS182_1.gfr\n";
    exit;
}
    ...
    for ($j=0;$j<@samfile;$j++){  #for loop of the FJR file
        ...
        if ($samfile[$j] =~m/\t$T1[0]\t/ && $samfile[$j] =~m/\t$T2[0]\t/ | $samfile[$j] =~m/\t=\t/){
            ...
            if (($test[3]>$T1[1] && $test[3]<$T1[2] && $test[7]>$T2[1] && $test[7]<$T2[2]) | ($test[7]>$T1[1] && $test[7]<$T1[2] && $test[3]>$T2[1] && $test[3]<$T2[2])){
                ...
                #print "hey~ @test\n";
                #print "before $test[0]\n";
                my @tmpProce = Seq_seqProcess(\@gfr, \@test);            
                #print "after $tmpProce[0]\n";
                #print "return $tmpProce[2]\n";
                push (@{$FJR{$geneID}}, ["$tmpProce[0]", "$tmpProce[1]", "$tmpProce[2]", "$tmpProce[3]", "$tmpProce[4]", "$tmpProce[5]", "$tmpProce[6]"]);

            #     if ( ($test[2]=~m/$gfr[11]/) && ($test[6]=~m/=/ | $test[6]=~m/$gfr[11]/) ){
            #         if ($test[5]=~m/[0-9][0-9]M[0-9][0-9]S/){
            #             #print "typical0_1 $test[0] $test[2] $test[3] $test[5] $test[2] $test[7] $test[9]\n";
            #             push (@{$FJR{$geneID}}, ["$test[0]", "$test[2]", "$test[3]", "$test[5]", "$test[2]", "$test[7]", "$test[9]"]);   ##intra case, $test[6] is "="
            #         }
            #         if ($test[5]=~m/[0-9][0-9]S[0-9][0-9]M/){
            #             $test[9]=DNA_anti(scalar reverse ($test[9]));
            #             my @tmp = split /[MS]/, $test[5];
            #             $test[5]="$tmp[1]M$tmp[0]S";
            #             #print "typical0_2 $test[0] $test[2] $test[3] $test[5] $test[2] $test[7] $test[9]\n";
            #             push (@{$FJR{$geneID}}, ["$test[0]", "$test[2]", "$test[3]", "$test[5]", "$test[2]", "$test[7]", "$test[9]"]);
            #         }
            #     }
            #     elsif ($test[2]=~m/$gfr[11]/ && ($test[6]!~m/=/)){
            #         if ($test[5] =~ m/[0-9][0-9]M[0-9][0-9]S/){
            ...
            #         }
            #         #push (@{$FJR{$geneID}}, ["$test[0]", "$test[2]", "$test[3]", "$test[5]", "$test[6]", "$test[7]", "$test[9]"]);     ##inter case, $test[2] is $gfr[11],$test[6] is not "="
            #     }
            #     else{
            #         #print "typical2_1 @test\n";
            #         $test[9]=DNA_anti(scalar reverse ($test[9]));
            ...
            #         if ($test[5] =~ m/[0-9][0-9]M[0-9][0-9]S/){
				        ...
				        # $test[5]="$tmp[1]M$tmp[0]S";
				        # if ($test[6]!~ m/=/){
					       # #print "typical2_2 $test[0] $test[6] $test[7] $test[5] $test[2] $test[3] $test[9]\n" ;
					       # push (@{$FJR{$geneID}}, ["$test[0]", "$test[6]", "$test[7]", "$test[5]", "$test[2]", "$test[3]", "$test[9]"]);
				        # }
				        # else{
					       # #print "typical2_2 $test[0] $test[2] $test[7] $test[5] $test[2] $test[3] $test[9]\n" ;
					       # push (@{$FJR{$geneID}}, ["$test[0]", "$test[2]", "$test[7]", "$test[5]", "$test[2]", "$test[3]", "$test[9]"]);
				        # }
			         # }
            #         if ($test[5] =~ m/[0-9][0-9]S[0-9][0-9]M/){
				        # my @tmp = split /[MS]/, $test[5];
				        # $test[5]="$tmp[0]M$tmp[1]S";
				        # if ($test[6]!~ m/=/){
            #                 #print "typical2_2 $test[0] $test[6] $test[7] $test[5] $test[2] $test[3] $test[9]\n" ;
            #                 push (@{$FJR{$geneID}}, ["$test[0]", "$test[6]", "$test[7]", "$test[5]", "$test[2]", "$test[3]", "$test[9]"]);
            #             }
            ...
            for ($j=0; $j<@all_tmp; $j++){
                ...
                for ($k=0; $k< @value; $k++){
                    if (($test[9] =~ m/$value[$k]/) | ($test[9] =~ m/DNA_anti(scalar reverse ($value[$k]))/) ){
                        #print "before $test[0]\n";
                        my @tmpProce2 = Seq_seqProcess(\@gfr, \@test);
                        #print "after @tmpProce2\n";
                        if (@tmpProce2){
                            #print "$tmpProce2[0], $tmpProce2[1], $tmpProce2[2], $tmpProce2[3], $tmpProce2[4], $tmpProce2[5], $tmpProce2[6]\n";
                            push (@{$FJR{$geneID}}, ["$tmpProce2[0]", "$tmpProce2[1]", "$tmpProce2[2]", "$tmpProce2[3]", "$tmpProce2[4]", "$tmpProce2[5]", "$tmpProce2[6]"]);    
                        }
                ...
                #             if ($test[2] =~ m/$gfr[11]/){
                #                 push (@{$FJR{$geneID}}, ["$test[0]", "$test[2]", "$test[3]", "$test[5]", "$test[6]", "$test[7]", "$test[9]"]);
                #                 #print "rescu1! $test[0] $test[2] $test[3] $test[5] $test[6] $test[7] $test[9]\n";
                #             }
                #             else {
                #                 $test[9]=DNA_anti(scalar reverse ($test[9]));
                #                 my @tmp = split /[MS]/, $test[5];
                #                 $test[5]="$tmp[1]M$tmp[0]S";
                #                 push (@{$FJR{$geneID}}, ["$test[0]", "$test[6]", "$test[7]", "$test[5]", "$test[2]","$test[3]", "$test[9]"]);
                #                 #print "rescu2! $test[0] $test[6] $test[7] $test[5] $test[2] $test[3] $test[9]\n";
                #             }
...
#sub Seq_seqProcess(\@\@){    
    ...
    @gfr=@$gfr_ref;
    @test=@$test_ref;
    #print "GFR $gfr[11]\nTEST $test[2] $test[5] $test[6]\n";
    #print "$test[2] $test[6] $gfr[11]\n";
    if ( ($test[2]=~m/$gfr[11]/) && ($test[6]=~m/=/ | $test[6]=~m/$gfr[11]/) ){
        if ($test[5]=~m/[0-9][0-9]M[0-9][0-9]S/){
                #print "typical0_1 $test[0] $test[2] $test[3] $test[5] $test[2] $test[7] $test[9]\n";
                @processed_seq=("$test[0]", "$test[2]", "$test[3]", "$test[5]", "$test[2]", "$test[7]", "$test[9]");
    ...
    # elsif ($test[2]=~m/$gfr[11]/ && ($test[6]!~m/=/)){
    #     if ($test[5] =~ m/[0-9][0-9]M[0-9][0-9]S/){
    #         @processed_seq=("$test[0]", "$test[2]", "$test[3]", "$test[5]", "$test[6]", "$test[7]", "$test[9]");
    #         #push (@{$FJR{$geneID}}, ["$test[0]", "$test[2]", "$test[3]", "$test[5]", "$test[6]", "$test[7]", "$test[9]"]);
    #     }
    #     if ($test[5] =~ m/[0-9][0-9]S[0-9][0-9]M/){
    #         $test[9]=DNA_anti(scalar reverse ($test[9]));
    ...
    #     }
        ...
        if ($test[5] =~ m/[0-9][0-9]M[0-9][0-9]S/){
            ...
            if ($test[6]!~ m/=/){
                #print "typical2_2 $test[0] $test[6] $test[7] $test[5] $test[2] $test[3] $test[9]\n" ;
                @processed_seq=("$test[0]", "$test[6]", "$test[7]", "$test[5]", "$test[2]", "$test[3]", "$test[9]");
                
                #push (@{$FJR{$geneID}}, ["$test[0]", "$test[6]", "$test[7]", "$test[5]", "$test[2]", "$test[3]", "$test[9]"]);
            }
            else{
                #print "typical2_2 $test[0] $test[2] $test[7] $test[5] $test[2] $test[3] $test[9]\n" ;
                @processed_seq=("$test[0]", "$test[2]", "$test[7]", "$test[5]", "$test[2]", "$test[3]", "$test[9]");
    ...
    #print "@processed_seq\n";
    # if (@processed_seq){
    #     print "processed_seq has value\n";
    # }
    # if (!@processed_seq){
    #     print "processed_seq has NO value\n";
    # }
