...
#echo "$OUTPUT_DIR/$(basename $l .list)/$SAMPLEID""_Aligned.out.bam"
	...
	if [[ -s $OUTPUT_DIR/$(basename $l .list)/$SAMPLEID.sorted.bam.bai ]];then ## the new STAR processing deletes the Aligned.out files
	    continue; 
	fi
#	SECOND_READ=""
