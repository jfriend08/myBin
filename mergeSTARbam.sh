...
while getopts "i:p:l:b:FMS" optname
  ...
  case "$optname" in
	  ...
	  ;;
      "F")
	  echo "" "sorted.bam file will be regenerated"
	  FORCE="TRUE"
	  ;;
      "l")
	  LABEL=$OPTARG
...
do
    ...
    SAMPLE=$(basename "$ROOT_DIR");#echo "INFO" "ROOT_DIR\t$ROOT_DIR"
    BAMCOUNT=$(find $ROOT_DIR -type f -size +0 -name "*_Aligned.out.$SAMSUFFIX" | wc -l );#echo "INFO" "There is/are $BAMCOUNT SAM/BAM files."
#    if [[ $BAMCOUNT -ne 1 ]];then
#	continue;
#    fi	
    BAMFILES=$(find $ROOT_DIR -type f -size +0 -name "*_Aligned.out.$SAMSUFFIX");#echo "INFO" "BAMFILES:\n$BAMFILES"
    for BAMFILE in $BAMFILES 
    do
	MAIN_DIR=$(dirname $BAMFILE);#echo "INFO" "MAIN_DIR:\t$MAIN_DIR"
	if [[ ! -e $BAMFILE ]];then
	    echo "ERROR" "BAM output doesn't exists:***\t$BAMFILE\t****"
	    exit -2
	fi
	SAMPLEID=$(basename "$BAMFILE" | sed 's%\(.*\)_Aligned.out.[bs]am%\1%');#echo "INFO" "SampleIDs:\t$SAMPLEID" ## to add [bs]am when completed
	...
	SAM_FLAG=$(basename $BAMFILE | sed 's%\(.*\)_Aligned.out.\([bs]am\)%\2%');#echo "INFO" "SAM_FLAG ($SAMPLEID):\t$SAM_FLAG"
	...
	SAM=$(basename $BAMFILE);#echo "INFO" "$SAM"
	CHIM=$SAMPLEID"_Chimeric.out.sam";#echo "INFO" "$CHIM"
	CHIMB=$SAMPLEID"_Chimeric.out.bam";#echo "INFO" "$CHIMB"	
	
	JUNC=$SAMPLEID"_Chimeric.out.junction";#echo "INFO" "$JUNC"
	if [[ (! -e $MAIN_DIR/$CHIM) && ( ! -e  $MAIN_DIR/$CHIMB ) ]]; then
	    echo "ERROR" "Chimeric sam file does not exists. Skipping:\n$MAIN_DIR/$CHIM\n$MAIN_DIR/$CHIMB"
	    continue
	fi
	if [[ ! -e $MAIN_DIR/$JUNC ]]; then
	    echo "ERROR" "Chimeric junction file does not exists. Skipping"
	    continue
	fi
### with PICARD MergeSam - deprecated	echo "cd $ROOT_DIR; (grep 'p' $JUNC | cut -f10 > $SAMPLEID""_IDlist.txt); Split_FJR.pl $CHIM $SAMPLEID""_IDlist.txt $ROOT_DIR &> $SAMPLEID""_split.log; (samtools view -@ 8 -bh $SAM_FLAG -F 256 $SAM ) > $SAMPLEID""_filteredAlign.bam 2> $SAMPLEID""_filteredAlign.log; (samtools view -@ 8 -Sbh -F 256 $SAMPLEID""_chimericPE.sam) > $SAMPLEID""_chimericPE_filtered.bam 2> $SAMPLEID""_chimericPE_filtered.log; (java -Xmx16g -jar $PICARD_DIR/MergeSamFiles.jar MERGE_SEQUENCE_DICTIONARIES=true USE_THREADING=false MAX_RECORDS_IN_RAM=5000000 INPUT=$SAMPLEID""_filteredAlign.bam INPUT=$SAMPLEID""_chimericPE_filtered.bam SORT_ORDER=queryname VALIDATION_STRINGENCY=LENIENT TMP_DIR=./$SAMPLEID""_tmp OUTPUT=$SAMPLEID""_Merged.bam ) 2> $SAMPLEID.MergeSam.log; (samtools view -@ 8 $SAMPLEID""_Merged.bam | sam2mrf | gzip ) > $SAMPLEID.mrf.gz 2> $SAMPLEID.mrf.log; (~/bin/samtools sort -@ 8 -m 2G $SAMPLEID""_Merged.bam $SAMPLEID"".sorted) 2> $SAMPLEID"".sorted.log; samtools index $SAMPLEID"".sorted.bam; if [[ \$(samtools view -c $SAMPLEID""_Merged.bam) != \$(samtools view -c $SAMPLEID"".sorted.bam) ]];then printf \"$SAMPLEID\\tNOT OK\" > $SAMPLEID.sorting.check; else printf \"$SAMPLEID\\tOK\\nRemoving $SAMPLEID""_Merged.bam and associated files\" > $SAMPLEID.sorting.check; rm -r $SAMPLEID""_Merged.bam $SAMPLEID""_filteredAlign.bam $SAMPLEID""_chimericPE_filtered.bam $SAMPLEID""_tmp; fi;samtools view -@ 8 -Shb $SAMPLEID""_FJR.sam > $SAMPLEID""_FJR.tmp.sam; ~/bin/samtools sort -@ 8 $SAMPLEID""_FJR.tmp.sam $SAMPLEID""_FJR  2> $SAMPLEID""_FJR.bam.log; samtools index $SAMPLEID""_FJR.bam; (samtools view -@ 8 -Shb $SAMPLEID""_Chimeric.out.sam > $SAMPLEID""_Chimeric.out.bam ) 2> $SAMPLEID""_Chimeric.out.bam.log; if [[ \$(samtools view -c $SAMPLEID""_Chimeric.out.bam) -eq \$(samtools view -Sc $SAMPLEID""_Chimeric.out.sam) ]];then mv $SAMPLEID""_Chimeric.out.sam $SAMPLEID""_Chimeric.out.sam.old ; fi;rm $SAMPLEID""_FJR.sam;"      >> $JOBLIST
	if [[ $CREATE_MRF == "TRUE" ]];then
	    echo "cd $MAIN_DIR; (grep 'p' $JUNC | cut -f10 > $SAMPLEID""_IDlist.txt); Split_FJR.pl $CHIM $SAMPLEID""_IDlist.txt $MAIN_DIR &> $SAMPLEID""_split.log; (samtools view -@ 8 -bh $SAM_FLAG -F 256 $SAM ) > $SAMPLEID""_filteredAlign.bam 2> $SAMPLEID""_filteredAlign.log; (samtools view -@ 8 -Sbh -F 256 $SAMPLEID""_chimericPE.sam) > $SAMPLEID""_chimericPE_filtered.bam 2> $SAMPLEID""_chimericPE_filtered.log; samtools view $SAM_FLAG -H $SAM > $SAM.header; (samtools merge -n -@ 8 -h $SAM.header $SAMPLEID""_Merged.bam $SAMPLEID""_filteredAlign.bam $SAMPLEID""_chimericPE_filtered.bam) 2> $SAMPLEID.MergeSam.log; (samtools view -@ 8 $SAMPLEID""_Merged.bam | sam2mrf | gzip ) > $SAMPLEID.mrf.gz 2> $SAMPLEID.mrf.log; (~/bin/samtools sort -@ 8 -m 2G $SAMPLEID""_Merged.bam $SAMPLEID"".sorted) 2> $SAMPLEID"".sorted.log; samtools index $SAMPLEID"".sorted.bam; if [[ \$(samtools view -c $SAMPLEID""_Merged.bam) != \$(samtools view -c $SAMPLEID"".sorted.bam) ]];then printf \"$SAMPLEID\\tNOT OK\" > $SAMPLEID.sorting.check; else printf \"$SAMPLEID\\tOK\\nRemoving $SAMPLEID""_Merged.bam and associated files\" > $SAMPLEID.sorting.check; rm -r $SAM.header $SAMPLEID""_Merged.bam $SAMPLEID""_filteredAlign.bam $SAMPLEID""_chimericPE_filtered.bam $SAMPLEID""_tmp; fi;samtools view -@ 8 -Shb $SAMPLEID""_FJR.sam > $SAMPLEID""_FJR.tmp.bam; ~/bin/samtools sort -@ 8 $SAMPLEID""_FJR.tmp.bam $SAMPLEID""_FJR  2> $SAMPLEID""_FJR.bam.log; samtools index $SAMPLEID""_FJR.bam; (samtools view -@ 8 -Shb $SAMPLEID""_Chimeric.out.sam > $SAMPLEID""_Chimeric.out.bam ) 2> $SAMPLEID""_Chimeric.out.bam.log; if [[ \$(samtools view -c $SAMPLEID""_Chimeric.out.bam) -eq \$(samtools view -Sc $SAMPLEID""_Chimeric.out.sam) ]];then mv $SAMPLEID""_Chimeric.out.sam $SAMPLEID""_Chimeric.out.sam.old ; fi;rm $SAMPLEID""_FJR.sam;"      >> $JOBLIST
	else
	    echo "cd $MAIN_DIR; (grep 'p' $JUNC | cut -f10 > $SAMPLEID""_IDlist.txt); Split_FJR.pl $CHIM $SAMPLEID""_IDlist.txt $MAIN_DIR &> $SAMPLEID""_split.log; (samtools view -@ 8 -bh $SAM_FLAG -F 256 $SAM ) > $SAMPLEID""_filteredAlign.bam 2> $SAMPLEID""_filteredAlign.log; (samtools view -@ 8 -Sbh -F 256 $SAMPLEID""_chimericPE.sam) > $SAMPLEID""_chimericPE_filtered.bam 2> $SAMPLEID""_chimericPE_filtered.log; samtools view $SAM_FLAG -H $SAM > $SAM.header; (samtools merge -@ 8 -h $SAM.header $SAMPLEID""_Merged.bam $SAMPLEID""_filteredAlign.bam $SAMPLEID""_chimericPE_filtered.bam) 2> $SAMPLEID.MergeSam.log; ~/bin/samtools sort $SAMPLEID""_Merged.bam $SAMPLEID.sorted; samtools index $SAMPLEID"".sorted.bam;  if [[ \$(samtools view -c $SAMPLEID""_Merged.bam) != \$(samtools view -c $SAMPLEID"".sorted.bam) ]];then printf \"$SAMPLEID\\tNOT OK\" > $SAMPLEID.sorting.check; else printf \"$SAMPLEID\\tOK\\nRemoving $SAMPLEID""_Merged.bam and associated files\" > $SAMPLEID.sorting.check; rm -r $SAM.header $SAMPLEID""_Merged.bam $SAMPLEID""_filteredAlign.bam $SAMPLEID""_chimericPE_filtered.bam $SAMPLEID""_tmp; fi; rm -r $SAM.header $SAMPLEID""_filteredAlign.bam $SAMPLEID""_chimericPE_filtered.bam;samtools view -@ 8 -Shb $SAMPLEID""_FJR.sam > $SAMPLEID""_FJR.tmp.bam; ~/bin/samtools sort -@ 8 $SAMPLEID""_FJR.tmp.bam $SAMPLEID""_FJR  2> $SAMPLEID""_FJR.bam.log; samtools index $SAMPLEID""_FJR.bam; (samtools view -@ 8 -Shb $SAMPLEID""_Chimeric.out.sam > $SAMPLEID""_Chimeric.out.bam ) 2> $SAMPLEID""_Chimeric.out.bam.log; if [[ \$(samtools view -c $SAMPLEID""_Chimeric.out.bam) -eq \$(samtools view -Sc $SAMPLEID""_Chimeric.out.sam) ]];then mv $SAMPLEID""_Chimeric.out.sam $SAMPLEID""_Chimeric.out.sam.old ; fi;rm $SAMPLEID""_FJR.sam;"      >> $JOBLIST
	fi
    done
